...
class MLD(BaseModel):
    ...
    def __init__(self, cfg, datamodule, **kwargs):
        ...
        if self.stage == "diffusion":
            if self.vae_type in ["vq","mld", "vposert","actor"]:
        ...
        if cfg.LOSS.TYPE == "mld":
            self._losses = MetricCollection({
                split: MLDLosses(vae=self.is_vae, mode="xyz", cfg=cfg,nfeats = self.datamodule.nfeats)
    ...
    def forward(self, batch):
        ...
        with torch.no_grad():
            ...
            if self.vae_type in ["vq","mld","actor"]:
                feats_rst = self.vae.custom_decoder(z)
                print("\nFeats: ",feats_rst.size())
            elif self.vae_type == "no":
    ...
    def train_diffusion_forward(self, batch):
        ...
        with torch.no_grad():
            if self.vae_type in ["vq","mld", "vposert", "actor"]:
                z, dist = self.vae.custom_encode(feats_ref)
                print("\n\n\nIn Train Diffusion Forward")
                print("Features ref: ",feats_ref.size())
                print("Z: ",z.size())
            elif self.vae_type == "no":
    ...
    def t2m_eval(self, batch):
        ...
        if self.stage in ['diffusion', 'vae_diffusion']:
            ...
            if self.vae_type in ["vq","mld", "vposert", "actor"]:
                z, dist_m = self.vae.encode(motions)
            else:
        ...
        with torch.no_grad():
            if self.vae_type in ["vq","mld", "vposert", "actor"]:
                #feats_rst = self.vae.decode(z)
                feet, l, p = self.vae(motions)
            elif self.vae_type == "no":
                feats_rst = z.permute(1, 0, 2)
        ...
        # joints recover
        joints_rst = self.feats2joints(feet)
        joints_ref = self.feats2joints(motions)

        # renorm for t2m evaluators
        feats_rst = self.datamodule.renorm4t2m(feet)
